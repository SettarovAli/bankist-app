...
const animationLogo = function () {
  logotype.style.animation = '';
  setTimeout(function () {
    logotype.style.animation = 'logo 2s';
  }, 0);
};
...
const displayMovements = function (acc, sort = false) {
  ...
  const movementsRows = document.querySelectorAll(
    '.movements__row:nth-child(2n)'
  );

  Array.from(movementsRows).forEach(
    row => (row.style.backgroundColor = '#fefff2')
  );
};
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    const welcomeDate = new Date().getHours();
    // console.log(new Date(welcomeDate));
    if (welcomeDate >= 0 && welcomeDate < 6) {
      labelWelcome.textContent = `Good night, ${
        currentAccount.owner.split(' ')[0]
      }`;
    } else if (welcomeDate >= 6 && welcomeDate < 12) {
      labelWelcome.textContent = `Good morning, ${
        currentAccount.owner.split(' ')[0]
      }`;
    } else if (welcomeDate >= 12 && welcomeDate < 18) {
      labelWelcome.textContent = `Good afternoon, ${
        currentAccount.owner.split(' ')[0]
      }`;
    } else if (welcomeDate >= 18 && welcomeDate < 24) {
